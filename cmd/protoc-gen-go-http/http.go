...
// 初始化fileDesc对象
func (f *fileDesc) initialize(file *protogen.File, g *protogen.GeneratedFile) {
	f.Version = version
	f.SourceFilePath = file.Desc.Path()
	f.PackageName = string(file.GoPackageName)
	for _, service := range file.Services {
		f.initService(g, service, true)
	}
}

// 初始化service
func (f *fileDesc) initService(g *protogen.GeneratedFile, service *protogen.Service, omitempty bool) {
	sd := &serviceDesc{
...
func hasHTTPRule(services []*protogen.Service) bool {
	for _, srv := range services {
		for _, method := range srv.Methods {
			...
			exts := proto.GetExtension(opts, annotations.E_Http)
			rule, ok := exts.(*annotations.HttpRule)
			if rule != nil && ok {
				return true
			}
		}
