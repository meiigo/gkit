...
// go test -v *.go -test.run=^TestJSONMarshal$
...
// go test -v *.go -test.run=^TestJSONUnmarshal$
func TestJSONUnmarshal(t *testing.T) {
	...
	for _, v := range tests {
		want := []byte(v.input)
		err := (code{}).Unmarshal(want, &v.expect)
		if err != nil {
			t.Errorf("marshal(%#v): %s", v.input, err)
		}
		got, err := code{}.Marshal(v.expect)
		if err != nil {
			t.Errorf("marshal(%#v): %s", v.input, err)
		}
